...
def put_bar(rho, start_point, end_point, width):
    assert start_point[0] >= 0 and start_point[0] <= rho.shape[1], 'start_point x index {} must be 1~{}'.format(
        start_point[0], rho.shape[1])
    assert start_point[1] >= 0 and start_point[1] <= rho.shape[0], 'start_point y index {} must be 1~{}'.format(
        start_point[1], rho.shape[0])
    assert end_point[0] >= 0 and end_point[0] <= rho.shape[1], 'end_point x index {} must be 1~{}'.format(
        end_point[0], rho.shape[1])
    assert end_point[1] >= 0 and end_point[1] <= rho.shape[0], 'end_point y index {} must be 1~{}'.format(
